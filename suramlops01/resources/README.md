...
The databricks CLI bundles top level is defined by file `suramlops01/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`suramlops01/resources/model-workflow-resource.yml`)
 - batch inference workflow (`suramlops01/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`suramlops01/resources/monitoring-resource.yml`)
 - feature engineering workflow (`suramlops01/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`suramlops01/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `suramlops01/databricks.yml` and `suramlops01/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-suramlops01-model     | /dev-suramlops01-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest dev content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-suramlops01-model | /staging-suramlops01-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-suramlops01-model    | /prod-suramlops01-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the dev branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-suramlops01-model    | /test-suramlops01-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against dev branch.
...
Upon merging a PR to the dev branch, the dev branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
2. Complete the `TODO` in `suramlops01/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Open a pull request to merge the pushed branch into the `dev` branch.
...
Once CI passes, merge the PR into the `dev` branch. This will deploy an initial set of Databricks resources to the staging workspace.
...
Following this, update the batch_inference_job base parameters in `suramlops01/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `suramlops01/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
The example above defines a Databricks job with name `${bundle.target}-suramlops01-batch-inference-job`
that runs the notebook under `suramlops01/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-suramlops01-batch-inference-job`.
...
`test-suramlops01-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-suramlops01-batch-inference-job`.
...
After setting up CI/CD, PRs against the dev branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the dev branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from dev to release. Continuous deployment automation will deploy `prod` resources to the prod workspace.
