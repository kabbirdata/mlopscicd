...
Commit the current `README.md` file and other docs to the `dev` branch of the repo, to enable forking the repo:
```

git add README.md docs .gitignore suramlops01/resources/README.md
...
- **`suramlops01-tests-ci.yml`**:<br>
  - **[CI]** Performs unit and integration tests<br>
    - Triggered on PR to main
- **`suramlops01-bundle-cicd.yml`**:<br>
  - **[CI]** Performs validation of Databricks resources defined under `suramlops01/resources`<br>
...
- You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-701332733887587.7.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-2900826933759041.1.azuredatabricks.net#setting/accounts). If
...
1. Create a new variable group called `suramlops01 variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
...
1. Define [build validation branch policies](https://learn.microsoft.com/en-us/azure/devops/repos/git/branch-policies?view=azure-devops&tabs=browser#build-validation) for the `dev` branch using the Azure build pipelines created in step 1. This is required so that any PR changes to the `dev` must build successfully before PRs can complete. 
...
After the pull request merges, pull the changes back into your local `dev`
branch:
...
Create and push a release branch called `release` off of the `dev` branch of the repository:
```
...
Your production jobs (model training, batch inference) will pull ML code against this branch, while your staging jobs will pull ML code against the `dev` branch. Note that the `dev` branch will be the source of truth for ML resource configs and CI/CD workflows.

For future ML code changes, iterate against the `dev` branch and regularly deploy your ML code from staging to production by merging code changes from the `dev` branch into the `release` branch.

## Deploy ML resources and enable production jobs
Follow the instructions in [suramlops01/resources/README.md](../suramlops01/resources/README.md) to deploy ML resources
and production jobs.
...
[ML resource config guide](../suramlops01/resources/README.md)  to propose, test, and deploy
